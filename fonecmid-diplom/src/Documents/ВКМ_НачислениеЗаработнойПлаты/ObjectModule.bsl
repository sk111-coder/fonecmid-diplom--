...
Процедура СформироватьДвиженияПоПремииПроцентом()
	...
	СоответствиеПроцентов = Новый Соответствие;
...
|ИЗ
|	РегистрСведений.ВКМ_УсловияОплатыСотрудников.СрезПоследних(&ДатаДокумента, Сотрудник В (&СписокСотрудников)) КАК ВКМ_УсловияОплатыСотрудниковСрезПоследних";
	
	ЗапросПроцент.УстановитьПараметр("ДатаДокумента", Дата);
	ЗапросПроцент.УстановитьПараметр("СписокСотрудников", ДополнительныеНачисления.Выгрузить(, "Сотрудник"));
	
	РезультатПроцент = ЗапросПроцент.Выполнить();
	ВыборкаПроцент = РезультатПроцент.Выбрать();
	Пока ВыборкаПроцент.Следующий() Цикл
		СоответствиеПроцентов.Вставить(ВыборкаПроцент.Сотрудник, ВыборкаПроцент.ПроцентОтРабот);
	КонецЦикла;
	
	// Формируем движения с процентами
	Для Каждого СтрокаТЧ Из ДополнительныеНачисления Цикл
		...
		// Заполняем процент из соответствия
		Если СоответствиеПроцентов.Получить(СтрокаТЧ.Сотрудник) <> Неопределено Тогда
			Движение.ПроцентПремии = СоответствиеПроцентов.Получить(СтрокаТЧ.Сотрудник);
		КонецЕсли;
	КонецЦикла;
