...
Процедура ЗаполнитьНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СуммаНачисления <> 0 И Выборка.ПроцентОтРабот <> 0 Тогда
	         НоваяСтрока = Объект.ДополнительныеНачисления.Добавить();
	         НоваяСтрока.Сотрудник = Выборка.Сотрудник;
	         НоваяСтрока.СуммаНачисления = Выборка.СуммаНачисления * Выборка.ПроцентОтРабот / 100;
	         НоваяСтрока.НачисленоЧасов = Выборка.НачисленоЧасов;
     КонецЕсли;
	КонецЦикла;
